...
{
    public class QuestionAnswerDal : EntityRepositoryBase<QuestionAnswer, ApplicationDbContext>, IQuestionAnswerDal
    {
        private readonly ApplicationDbContext _context;
        public QuestionAnswerDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public int QuestionAnswerCounter()
        {
            try
            {
                return _context.QuestionAnswers.Count();
            }
            catch (Exception)
            {
                return 0;
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<QuestionAnswer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<QuestionAnswer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
