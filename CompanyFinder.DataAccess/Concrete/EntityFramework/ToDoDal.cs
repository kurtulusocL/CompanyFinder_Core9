...
namespace CompanyFinder.DataAccess.Concrete.EntityFramework
{
    public class ToDoDal : EntityRepositoryBase<ToDo, ApplicationDbContext>, IToDoDal
    {
        ...
        public ToDoDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public ToDo HitRead(int? id)
        {
            try
            {
                ...
                var hitRead = _context.Set<ToDo>().Where(i => i.Id == id).FirstOrDefault();
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<ToDo>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<ToDo>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
        ...
        public async Task<bool> SetFinishedAsync(int id)
        {
            try
            {
                var isFinished = await _context.Set<ToDo>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (isFinished != null)
                {
                    isFinished.IsFinished = true;
                    isFinished.FinishedDate = DateTime.Now.ToLocalTime();
                    await _context.SaveChangesAsync();
                    return true;
                }
                return false;
            ...
            {
                throw new Exception("An unexpected error occurred while setting Finished the entity.", ex);
            }
        }
        public async Task<bool> SetNotFinishedAsync(int id)
        {
            ...
            {
                throw new Exception("An unexpected error occurred while setting NotFinished the entity.", ex);
            }
        }
    }
}
