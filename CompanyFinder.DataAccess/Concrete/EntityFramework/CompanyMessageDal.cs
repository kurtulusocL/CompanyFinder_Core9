...
{
    public class CompanyMessageDal : EntityRepositoryBase<CompanyMessage, ApplicationDbContext>, ICompanyMessageDal
    {
        private readonly ApplicationDbContext _context;
        public CompanyMessageDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<CompanyMessage>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<CompanyMessage>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
