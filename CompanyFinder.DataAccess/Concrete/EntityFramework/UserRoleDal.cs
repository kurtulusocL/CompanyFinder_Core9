...
{
    public class UserRoleDal : EntityRepositoryBase<UserRole, ApplicationDbContext>, IUserRoleDal
    {
        ...
        public async Task<bool> SetActiveAsync(string id)
        {
            try
            {
                var active = await _context.Set<UserRole>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(string id)
        {
            try
            {
                var deleted = await _context.Set<UserRole>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
