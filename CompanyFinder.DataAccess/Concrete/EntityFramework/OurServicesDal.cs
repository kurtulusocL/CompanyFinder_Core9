...
{
    public class OurServicesDal : EntityRepositoryBase<OurServices, ApplicationDbContext>, IOurServicesDal
    {
        private readonly ApplicationDbContext _context;
        public OurServicesDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<OurServices>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<OurServices>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
