...
{
    public class ProductSubcategoryDal : EntityRepositoryBase<ProductSubcategory, ApplicationDbContext>, IProductSubcategoryDal
    {
        private readonly ApplicationDbContext _context;
        public ProductSubcategoryDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<ProductSubcategory>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<ProductSubcategory>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
