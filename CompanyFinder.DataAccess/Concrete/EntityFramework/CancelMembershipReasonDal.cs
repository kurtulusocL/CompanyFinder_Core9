...
{
    public class CancelMembershipReasonDal : EntityRepositoryBase<CancelMembershipReason, ApplicationDbContext>, ICancelMembershipReasonDal
    {
        private readonly ApplicationDbContext _context;
        public CancelMembershipReasonDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<CancelMembershipReason>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<CancelMembershipReason>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
