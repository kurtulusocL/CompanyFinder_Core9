...
{
    public class CustomerDal : EntityRepositoryBase<Customer, ApplicationDbContext>, ICustomerDal
    {
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<Customer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<Customer>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
