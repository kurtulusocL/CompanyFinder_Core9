...
{
    public class CancelMembershipDal : EntityRepositoryBase<CancelMembership, ApplicationDbContext>, ICancelMembershipDal
    {
        private readonly ApplicationDbContext _context;
        public CancelMembershipDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetCancelledAsync(int id)
        {
            try
            {
                var cancelled = await _context.Set<CancelMembership>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (cancelled != null)
                {
                    cancelled.IsCancelled = true;
                    cancelled.CancelDate = DateTime.Now.ToLocalTime();
                    await _context.SaveChangesAsync();
                    return true;
                }
                return false;
            ...
            {
                throw new Exception("An unexpected error occurred while setting Cancelled the entity.", ex);
            }
        }
