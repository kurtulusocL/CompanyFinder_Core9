...
{
    public class StockDal : EntityRepositoryBase<Stock, ApplicationDbContext>, IStockDal
    {
        private readonly ApplicationDbContext _context;
        public StockDal(ApplicationDbContext context) : base(context)
        {
            _context = context;
        }
        ...
        public async Task<bool> SetActiveAsync(int id)
        {
            try
            {
                var active = await _context.Set<Stock>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (active != null)
                {
        ...
        public async Task<bool> SetDeletedAsync(int id)
        {
            try
            {
                var deleted = await _context.Set<Stock>().Where(i => i.Id == id).FirstOrDefaultAsync();
                if (deleted != null)
                {
